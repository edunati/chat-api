...
class MessageAdded(graphene.ObjectType):
    '''
    Simple type representing a message response in subscription.
    '''
    id = graphene.Int(
    ...
    owner = graphene.Field(
        User,
        description='The owner of the current message'
    )


class StartTyping(graphene.ObjectType):
    '''
    Simple type representating User who is currently typing message.
    '''

    user = graphene.Field(
        User,
        description='The user who is currently typing'
    )
